...
export default function TeacherContent() {
  ...
  const loadData = useCallback(async () => {
    ...
    const facultyRes = await userService.getFacultiesByUser(userId);

    const myFaculty =
        facultyRes.data?.facult?.[0] ||
        facultyRes.data?.faculty?.[0] ||
        facultyRes.data?.[0];

    setTeacherFaculty(myFaculty);

    const categoryList = extractList(facultyRes).map(normalizeCategory);
    ...
    const allowedCategories =
        role === "ROLE_TEACHER"
            ? categoryList.filter(c => c.name === myFaculty)
            : categoryList;
    ...
    role === "ROLE_TEACHER"
        ...
        const contentResults = await Promise.allSettled(
            listToLoad.map(cat =>
                contentService.getByCategoryId(cat.id)
            )
        );
  ...
  const filtered = useMemo(() => content.filter((c) => {
    ...
    const teacherOwnContent =
        role !== "ROLE_TEACHER" ||
        c.category === teacherFaculty;
    ...
    <div className="content-page">
      ...
      <Modal open={showUpload} onClose={() => setShowUpload(false)} title="Upload Content" width="500px">
        <form onSubmit={handleUpload} className="upload-form">
          ...
          {JSON.parse(localStorage.getItem("roles") || "[]")[0]?.name === "ROLE_ADMIN" ? (
